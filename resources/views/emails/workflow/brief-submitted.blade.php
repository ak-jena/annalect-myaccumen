...
<table cellspacing="0" cellpadding="10" style="color:#666;font:13px Arial;line-height:1.4em;width:100%;">
	<tbody>
            ...
            <tr>
                <td>
                    <p> {{ $campaign->brief->user->name }} has submitted a new brief for {{ $campaign->brief->client->name }}, {{ $campaign->brief->campaign_name }}.</p>
                    <p>Access the brief on the link below to upload the targeting grid. This needs to be completed by <strong>{{ date('d/m/Y', strtotime($campaign->brief->brief_response_deadline)) }}</strong>. </p>
                    <p>
